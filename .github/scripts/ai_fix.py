...
def main():
    ...
    if cmd:
        ...
        if apply_code(files):
            ...
            post_comment("❌ 执行失败：未能从方案中提取到有效的代码。")
            return
...
"""
    ...
    if verdict and verdict.get("winner") in ["A", "B", "HYBRID"]:
        ...
        if apply_code(verdict.get("files", {})):
            msg = f"""
### 🤖 AI 自动修复结论 ({winner})
...
"""
        ...
        fallback_msg = f"""
### ⚖️ AI 仲裁未自动应用
系统无法 100% 确定最佳方案。请查看下方详细对比进行人工选择：
...
<details><summary>🔍 查看 方案 A (Qwen) 源代码</summary>{plan_a}
